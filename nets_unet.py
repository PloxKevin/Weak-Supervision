...
class img_decoder(nn.Module):
    ...
    def __init__(self):
        ...
        #check different multiple options:
        #make table with different networks.
...
class vae_road_layout(nn.Module):

    def __init__(self, with_msk_channel):
        super(vae_road_layout, self).__init__()
...
def loss_function_road_pred(pred, gt):

    loss_raw = F.mse_loss(pred, gt, reduction='none')

    index = torch.ones_like(loss_raw)
    index[:, :, 21:43, 21:43] = 0.

    loss = torch.mean(loss_raw*index)*(64*64-22*22)/(64*64)


    # smooth_loss = smooth_loss_one_layer(pred_map.view(-1, 1, 64, 64))

    return loss #+ 0.1 * smooth_loss


def loss_function_road_layout(pred, gt):
    CE = F.mse_loss(pred, gt, reduction='none')
    CE = torch.mean(CE)
...
def loss_function_pre_selection(pred_map, map):
    ...
    CE = F.mse_loss(pred_map, map.view(-1, 1), reduction='none')
